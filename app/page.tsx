...
type WorkoutPlan = {
  days: {
    ...
    sessions: {
      type: "fuerza" | "cardio";
      muscle_group?: string;
      exercises?: { name: string; sets: number; reps: string; rest_sec: number }[];
      duration_min?: number;
      intensity?: string;
    }[];
  }[];
...
function sumNutrients(
  items: { kcal: number; protein_g: number; fat_g: number; carbs_g: number }[]
) {
  return items.reduce(
    (acc, it) => ({
...
function generateDietPlan(opts: {
  ...
  const vegFruit = pool.filter((f) =>
    ["broccoli", "spinach", "apple", "banana"].some((k) =>
  ...
  const meals = shares.map((m) => {
    ...
    // resto de kcal: 75% carbos, 25% grasa (heurística)
    let kcalRem = Math.max(0, kcalT - nutP.kcal);
    const gramsC = roundTo(
      (((kcalRem * 0.75) / 4) / (C.per100.carbs_g / 100)) || 0,
      5
    );
    const gramsF = roundTo(
      (((kcalRem * 0.25) / 9) / (F.per100.fat_g / 100)) || 0,
      5
    );
...
// ------ generador ENTRENAMIENTO (reglas, sin IA) ------
function generateWorkoutPlan(opts: {
  goal: Goal;
  daysPerWeek?: number;
}): WorkoutPlan {
  ...
  const cardioBlock = (mins: number, intensity: string) => ({
    type: "cardio" as const,
    duration_min: mins,
    intensity,
  });
  const strengthBlock = (t: (typeof strengthTemplates)[number]) => ({
    type: "fuerza" as const,
    muscle_group: t.muscle_group,
    exercises: t.ex.map(([name, reps]) => ({
      name,
      sets: parseInt(reps.split("x")[0]) || 3,
      reps,
      rest_sec: 90,
    })),
  });
  ...
  const planDays = selected.map((day, i) => {
    ...
    const sessions: WorkoutPlan["days"][number]["sessions"] = [strengthBlock(t)];
...
function useCalorieLog(date: string) {
  ...
  function addFromPer100(
    name: string,
    per100: { kcal: number; protein_g: number; fat_g: number; carbs_g: number },
    grams: number
  ) {
    const f = grams / 100;
...
export default function Page() {
  ...
  const targets = useMemo(
    () => computeTargets(sex, age, height, weight, activity, goal),
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setStatus("usda");
        const r = await fetch(`/api/usda/search?q=${encodeURIComponent(q)}`, {
          signal: ctl.signal,
        });
  ...
  function onGenerateWorkout() {
    setWorkoutPlan(generateWorkoutPlan({ goal, daysPerWeek: 5 }));
  }
  ...
  return (
    <main style={wrap}>
      <h1 style={{ fontSize: 28, marginBottom: 8 }}>
        Fitness MVP (No AI) — with USDA calories
      </h1>
      <p style={{ marginTop: 0, opacity: 0.82 }}>
        Tabs: Calorie log • Diet plan • Workout plan
      </p>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        ...
        <button onClick={() => setTab("diet")} style={tabBtn(tab === "diet")}>
          Diet plan
        </button>
        <button
          onClick={() => setTab("workout")}
          style={tabBtn(tab === "workout")}
        >
          Workout plan
        </button>
      </div>
        ...
        <section style={{ ...card, marginTop: 14 }}>
          ...
          >
            ...
            <div style={{ gridColumn: "span 2" }}>
              <label style={label}>Search food</label>
              <input
                value={foodQuery}
            ...
            {results.map((f, i) => (
              <div key={i} style={{ ...card, padding: 12 }}>
                ...
                <div style={{ opacity: 0.8, fontSize: 12, margin: "6px 0" }}>
                  per 100g — kcal {f.per100.kcal}, P {f.per100.protein_g}g, F{" "}
                  {f.per100.fat_g}g, C {f.per100.carbs_g}g
                </div>
                <button
                  style={btn}
                  onClick={() => addFromPer100(f.name, f.per100, grams)}
                >
                  Add {grams} g
                </button>
              </div>
      ...
      {tab === "diet" && (
        <section style={{ ...card, marginTop: 14 }}>
          ...
          <div style={{ marginTop: 12 }}>
            <button onClick={onGenerateDiet} style={btn}>
              Generate diet
            </button>
          </div>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <pre style={{ ...card, whiteSpace: "pre-wrap" }}>
      ...
      {tab === "workout" && (
        <section style={{ ...card, marginTop: 14 }}>
          ...
          <button onClick={onGenerateWorkout} style={btn}>
            Generate workout
          </button>
          {workoutPlan && (
            <div style={{ marginTop: 12 }}>
...
function tabBtn(active: boolean): React.CSSProperties {
  return {
    ...btn,
    background: active ? "#1d1d22" : "#141418",
    borderColor: active ? "#3a3a44" : "#2b2b2f",
  };
}
